...
#### 出題内容
1. Fragmentの基本を学ぶ - クエリの一部を再利用可能なフラグメントとして定義する
2. 複数のクエリでFragmentを共有する - 共通フィールドセットの再利用方法を実装
3. インラインFragmentを使用する - 特定の条件下で追加フィールドを取得する方法
4. Interfaceの基本を実装する - 共通フィールドを持つ型の抽象化
5. 複数の型で共通Interfaceを実装する - 商品とサービスの共通インターフェース
6. Interfaceを実装した型のクエリ方法 - 型に応じた適切なフィールド取得
7. Unionの基本を実装する - 異なる型の結果を単一フィールドで返す
8. 検索結果のUnion型を実装する - ユーザー、商品、記事など異なる型の検索結果
9. Fragment spreadsと型条件を使う - 型に応じて異なるフィールドを取得
10. 高度なスキーマ設計 - Interface、Union、Fragmentを組み合わせたAPI設計
