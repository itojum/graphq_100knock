...
## #30: 商品とカテゴリをまとめて新規登録せよ（仮想拡張）

### お題
新しいカテゴリを作成し、そのカテゴリに紐づく新しい商品も作成するMutationを連続して実行してください。

### 要件
- まず`createCategory` Mutationを実行してカテゴリを作る
- 次にそのカテゴリIDを使って`createProduct` Mutationを実行する
- どちらも1つのリクエスト内（ネストではない）
- 最低限返すフィールドは
  - カテゴリ: `id`, `name`
  - 商品: `id`, `name`, `price`
- 変数 `$categoryName`, `$productName`, `$description`, `$price` を使用する

### ヒント
- GraphQLでは**同じリクエスト内に複数のMutationを並列で書く**ことができる。
- それぞれに別名（エイリアス）をつけて実行すれば、結果を区別できる。
